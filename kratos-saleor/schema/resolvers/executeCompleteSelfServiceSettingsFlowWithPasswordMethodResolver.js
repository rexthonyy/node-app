...
const getData = ({flow, completeSelfServiceSettingsFlowWithPasswordMethodInput}) => {
    return new Promise((resolve, reject) => {
        ...
        checkSettingsFlow(resolve, reject, flow, password);
    });
}

function checkSettingsFlow(resolve, reject, flow, password){
    pgKratosQueries.getSelfServiceSettingsFlowById([flow], result => {
        ...
        getIdentityById(identityId, identity => {
            ...
            getIdentityCredentialsByIdentityCredentialTypeIdAndIdentityId(resolve, reject, settingsFlow, identity, password);
        });
    });
}

function getIdentityCredentialsByIdentityCredentialTypeIdAndIdentityId(resolve, reject, settingsFlow, identity, password){
    pgKratosQueries.getIdentityCredentialsByIdentityCredentialTypeIdAndIdentityId([IDENTITY_CREDENTIAL_TYPE_PASSWORD, identity.id], result1 => {
