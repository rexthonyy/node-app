...
const pool = new Pool({
    user: process.env.POSTGRES_SHIFTS_USER,
    host: process.env.POSTGRES_SHIFTS_HOST,
    database: process.env.POSTGRES_SHIFTS_DB,
    password: process.env.POSTGRES_SHIFTS_PASSWORD,
    port: process.env.POSTGRES_SHIFTS_PORT
});

pool.query('SELECT NOW()', (err, res) => {
    if (err) {
        console.log(`${process.env.POSTGRES_SHIFTS_DB} database initialization failed!!!`);
    } else {
        console.log(`${process.env.POSTGRES_SHIFTS_DB} database initialized successfully!!!`);
    }
});
