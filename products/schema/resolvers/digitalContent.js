...
module.exports = async(parent, args, context) => {
    return new Promise(async(resolve, reject) => {
        ...
        if (userHasAccess(authUser.userPermissions, permissions) || userPermissionGroupHasAccess(authUser.permissionGroups, permissions)) {
            resolve(getDigitalContent(args));
        } else {
...
function getDigitalContent(args) {
    return new Promise((resolve, reject) => {
        productQueries.getDigitalContent([args.id], "id=$1", async result => {
            if (result.err) return reject(JSON.stringify(result.err));
            if (result.res.length == 0) return reject(`Could'nt resolve id: ${args.id}`);
            let digitalContent = result.res[0];
            resolve(await getGraphQLDigitalContentById(digitalContent.id));
        });
    });
}