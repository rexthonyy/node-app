...
module.exports = async(parent, args, context) => {
    return new Promise(async resolve => {
        ...
        if (userHasAccess(authUser.userPermissions, accessPermissions) || userPermissionGroupHasAccess(authUser.permissionGroups, accessPermissions)) {
            resolve(await unassignWarehouseShippingZone(args));
        } else {
...
function unassignWarehouseShippingZone(args) {
...
function resolveWarehouseShippingZone(warehouseId, shippingZoneId) {
    return new Promise((resolve, reject) => {
        productQueries.deleteWarehouseShippingZones([warehouseId, shippingZoneId], "warehouse_id=$1 AND shippingzone_id=$2", result => {
