...
module.exports = async(parent, args, context) => {
    return new Promise(async resolve => {
        ...
        if (userHasAccess(authUser.userPermissions, accessPermissions) || userPermissionGroupHasAccess(authUser.permissionGroups, accessPermissions)) {
            resolve(await giftCardBulkDeactivate(authUser, args));
        } else {
...
function giftCardBulkDeactivate(authUser, args) {
    return new Promise(resolve => {
        ...
        ids.forEach(async id => {
            try {
                let { e } = await giftCardDeactivate(authUser, id);
...
function giftCardDeactivate(authUser, giftCardId) {
    return new Promise((resolve, reject) => {
        productQueries.getGiftCard([giftCardId], "id=$1", result => {
            ...
            if (result.res.length == 0) return reject(getGraphQLOutput("getGiftCard", `GiftCard not found: ${giftCardId}`, "NOT_FOUND").errors);
            let giftCard_ = result.res[0];
            if (!giftCard_.is_active) return reject(getGraphQLOutput("giftCard", `GiftCard already deactivated: ${giftCardId}`, "ALREADY_EXISTS").errors);
            productQueries.updateGiftCard([giftCardId, false], "is_active=$2", "id=$1", async result => {
                ...
                if (result.res.length == 0) return reject(getGraphQLOutput("updateGiftCard", `GiftCard not updated: ${giftCardId}`, "GRAPHQL_ERROR").errors);
