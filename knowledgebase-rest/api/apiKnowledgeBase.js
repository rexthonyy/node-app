...
router.post("/import",  Multer({storage: Multer.memoryStorage()}).single("import"), (req, res) => {
    ...
    let knowledge_base_article_delayed_jobs_rows = [];
    let knowledge_base_list_rows = [];
        ...
        if(row.length == 0 ){
            ...
            if(row.charAt(0) == "["){
                switch(row.trim()){
                    ...
                    case "[knowledge_base_article_delayed_jobs]":
                        flag = 8;
                    break;

                    case "[knowledge_base_lists]":
                        flag = 9;
                    break;
                ...
                if(index > 1){
                    ...
                    switch(flag){
                        ...
                        case 8:
                            knowledge_base_article_delayed_jobs_rows.push(columns);
                        break;

                        case 9:
                            knowledge_base_list_rows.push(columns);
                        break;
                    }
                }
            }
    ...
    let knowledge_base_article_delayed_jobs_id_map = {};
    let knowledge_base_list_id_map = {};
    ...
    importKnowledgeBaseData(res, knowledge_base_id_map, knowledge_bases_rows, () => {
        importKnowledgeBaseTranslations(res, knowledge_base_id_map, knowledge_base_translation_id_map, knowledge_base_translations_rows, () => {
            importKnowledgeBaseLists(res, knowledge_base_id_map, knowledge_base_list_id_map, knowledge_base_list_rows, () => {
                importKnowledgeBaseCategories(res, knowledge_base_id_map, knowledge_base_categories_id_map, knowledge_base_categories_rows, () => {
                    importKnowledgeBaseCategoryTranslations(res, knowledge_base_categories_id_map, knowledge_base_list_id_map, knowledge_base_category_translations_id_map, knowledge_base_category_translations_rows, () => {
                        importKnowledgeBaseCategoryDelayedJobs(res, knowledge_base_id_map, knowledge_base_category_translations_id_map, knowledge_base_category_delayed_jobs_id_map, knowledge_base_category_delayed_jobs_rows, () => {
                            importKnowledgeBaseArticles(res, knowledge_base_id_map, knowledge_base_categories_id_map, knowledge_base_articles_id_map, knowledge_base_articles_rows, () => {
                                importKnowledgeBaseArticlesTranslations(res, knowledge_base_id_map, knowledge_base_categories_id_map, knowledge_base_articles_id_map, knowledge_base_list_id_map, knowledge_base_article_translations_id_map, knowledge_base_article_translations_rows, () => {
                                    importKnowledgeBaseArticleDelayedJobs(res, knowledge_base_id_map, knowledge_base_articles_id_map, knowledge_base_article_translations_id_map, knowledge_base_article_delayed_jobs_id_map, knowledge_base_article_delayed_jobs_rows, () => {
...
function importKnowledgeBaseData(res, kb_id_map, kb_rows, cb){
    ...
    kb_rows.forEach(row => {
        let data = {
            ...
            icon: row[2],
            footer: row[3],
            created_at: new Date(Date.parse(row[4])).toUTCString(),
            homepage_layout: row[5],
            category_layout: row[6],
            active: row[7] == "true",
            front_page: row[9],
            position: Number(row[10]),
            ui_color: row[11],
            is_archived: row[12] == "true"
        };

        pgQueries.importKnowledgeBase(data, result => {
            if(result.err){
...
function importKnowledgeBaseTranslations(res, kb_id_map, kb_trans_id_map, kb_trans_rows, cb){
    ...
    kb_trans_rows.forEach(row => {
        ...
        pgQueries.importKnowledgeBaseTranslation(data, result => {
            if(result.err){
...
function importKnowledgeBaseLists(res, kb_id_map, kb_lis_id_map, kb_list_rows, cb){
    let numRows = kb_list_rows.length;
    let count = -1;

    kb_list_rows.forEach(row => {
        let data = {
            knowledge_base_id: kb_id_map[row[1]],
            list_type: row[2],
            title: row[3],
            position: Number(row[4]),
            created_at: new Date(Date.parse(row[5])).toUTCString()
        };

        pgQueries.importKnowledgeBaseList(data, result => {
            if(result.err){
                result.err.errorIndex = 8472343929281;
                return res.status(500).json(result.err);
            }

            kb_lis_id_map[row[0]] = result.res.id;
            checkComplete();
        });
    });
...
function importKnowledgeBaseCategories(res, kb_id_map,  kb_cat_id_map, kb_cat_rows, cb){
    ...
    kb_cat_rows.forEach(row => {
        let data = {
            knowledge_base_id: kb_id_map[row[1]],
            parent_id: Number(row[2]),
            position: Number(row[3]),
            created_at: new Date(Date.parse(row[4])).toUTCString(),
            updated_at: new Date(Date.parse(row[5])).toUTCString(),
            is_archived: row[6] == "true"
        };

        pgQueries.importKnowledgeBaseCategory(data, result => {
            if(result.err){
...
function importKnowledgeBaseCategoryTranslations(res, kb_cat_id_map, kb_lis_id_map, kb_cat_trans_id_map, kb_cat_trans_rows, cb){
    ...
    kb_cat_trans_rows.forEach(row => {
        let data = {
            ...
            category_id: kb_cat_id_map[row[3]],
            created_at: new Date(Date.parse(row[4])).toUTCString(),
            updated_at: new Date(Date.parse(row[5])).toUTCString(),
            ui_color: row[6],
            category_icon: row[7],
            ...
            publish_now: row[12] == "true",
            active: row[13] == "true",
            permission: row[14],
            update_metadata: row[15],
            group_name: row[16],
            is_delete_scheduled: row[17] == "true",
            is_update_scheduled: row[18] == "true",
            knowledge_base_id: Number(row[19]),
            is_archived: row[20] == "true",
            list_id: kb_lis_id_map[row[21]]
        };

        pgQueries.importKnowledgeBaseCategoryTranslation(data, result => {
            if(result.err){
...
function importKnowledgeBaseCategoryDelayedJobs(res, kb_id_map, kb_cat_trans_id_map, kb_cat_delayed_job_id_map, kb_cat_delayed_job_rows, cb){
    ...
    kb_cat_delayed_job_rows.forEach(row => {
        let data = {
            knowledge_base_category_translation_id: kb_cat_trans_id_map[row[1]],
            run_at: new Date(Date.parse(row[2])).toUTCString(),
            created_at: new Date(Date.parse(row[3])).toUTCString(),
            knowledge_base_id: kb_id_map[row[4]],
            publish_update_delete: row[5]
        };

        pgQueries.importKnowledgeBaseCategoryDelayedJob(data, result => {
            if(result.err){
                result.err.errorIndex = 847236541;
                return res.status(500).json(result.err);
            }
...
function importKnowledgeBaseArticles(res, kb_id_map, kb_cat_id_map, kb_cat_art_id_map, kb_art_rows, cb){
    ...
    kb_art_rows.forEach(row => {
        let data = {
            ...
            updated_at: new Date(Date.parse(row[3])).toUTCString(),
            created_at: new Date(Date.parse(row[4])).toUTCString(),
            knowledge_base_id: kb_id_map[row[5]],
            is_archived: row[6] == "true"
        };

        pgQueries.importKnowledgeBaseArticle(data, result => {
            if(result.err){
...
function importKnowledgeBaseArticlesTranslations(res, kb_id_map, kb_cat_id_map, kb_cat_art_id_map, kb_lis_id_map, kb_cat_art_trans_id_map, kb_art_trans_rows, cb){
    ...
    kb_art_trans_rows.forEach(row => {
        let data = {
            ...
            active: row[10] == "true",
            publish_now: row[11] == "true",
            update_metadata: row[12],
            is_delete_scheduled: row[13] == "true",
            is_update_scheduled: row[14] == "true",
            knowledge_base_id: kb_id_map[row[15]],
            is_archived: row[16] == "true",
            category_id: kb_cat_id_map[row[17]],
            ui_color: row[18],
            list_name: row[19],
            position: Number(row[20]),
            list_id: kb_lis_id_map[row[21]]
        };

        pgQueries.importKnowledgeBaseArticleTranslation(data, result => {
            if(result.err){
                result.err.errorIndex = 840036541;
                return res.status(500).json(result.err);
            }
...
function importKnowledgeBaseCategoryDelayedJobs(res, kb_id_map, kb_cat_art_id_map, kb_cat_art_trans_id_map, kb_art_delayed_job_id_map, kb_art_delayed_job_rows, cb){
    let numRows = kb_art_delayed_job_rows.length;
    let count = -1;

    kb_art_delayed_job_rows.forEach(row => {
        let data = {
            knowledge_base_id: kb_id_map[row[1]],
            knowledge_base_article_id: kb_cat_art_id_map[row[2]],
            knowledge_base_article_translation_id: kb_cat_art_trans_id_map[row[3]],
            run_at: new Date(Date.parse(row[4])).toUTCString(),
            created_at: new Date(Date.parse(row[5])).toUTCString(),
            publish_update_delete: row[6]
        };

        pgQueries.importKnowledgeBaseArticleDelayedJob(data, result => {
            if(result.err){
                result.err.errorIndex = 847236541;
                return res.status(500).json(result.err);
            }

            kb_art_delayed_job_id_map[row[0]] = result.res.id;
            checkComplete();
        });
    });
                    ...
                    exportKnowledgeBaseCategoryDelayedJobs(res, result, kb_id, result => {
                        exportKnowledgeBaseArticles(res, result, kb_id, result => {
                            exportKnowledgeBaseArticleTranslations(res, result, kb_id, result => {
                                exportKnowledgeBaseArticleDelayedJobs(res, result, kb_id, result => {
                                    exportKnowledgeBaseLists(res, result, kb_id, result => {
    ...
    pgQueries.listKnowledgeBaseCategoriesByKnowledgeBaseId(kb_id, result => {
        ...
        function checkComplete(){
            ...
            if(count == numCategories){
                ...
                data += "id,name,kb_locale_id,category_id,created_at,updated_at,ui_color,category_icon,title_tag,footer,keywords,meta_description,publish_now,active,permission,update_metadata,group_name,is_delete_scheduled,is_update_scheduled,knowledge_base_id,is_archived,list_id\n";
                kb_cat_translations.forEach(kb => {
                    data+= `${kb.id},${kb.name},${kb.kb_locale_id},${kb.category_id},${kb.created_at},${kb.updated_at},${kb.ui_color},${kb.category_icon},${kb.title_tag},${kb.footer},${kb.keywords},${kb.meta_description},${kb.publish_now},${kb.active},${kb.permission},${kb.update_metadata},${kb.group_name},${kb.is_delete_scheduled},${kb.is_update_scheduled},${kb.knowledge_base_id},${kb.is_archived},${kb.list_id}\n`;
                });
                data += "\n";
                cb(data);
            }
...
function exportKnowledgeBaseArticleDelayedJobs(res, data, kb_id, cb){
...
function exportKnowledgeBaseLists(res, data, kb_id, cb){
...
router.get("/listKnowledgeBases", (req, res) => {
    pgQueries.listKnowledgeBases(res, (res_, result) => {
        ...
        if(latestKnowledgebase == null) return getKnowledgebases(req, res, knowledgebases);
        ...
        setOrderForKnowledgebases(kb_ids, () => {
            getKnowledgebases(req, res, knowledgebases);
        });
    });
});

function getKnowledgebases(req, res, knowledgebases) {
    let front_pagefilteredKnowledgeBases = [];
    knowledgebases.forEach(kb => {
        if(req.query.front_page == null){
            front_pagefilteredKnowledgeBases.push(kb);
    ...
    function checkItIsComplete(){
        ...
        if(count1 == numFilters){
            ...
            return res.json(util.paginate(req, namefilteredKnowledgeBases));
        }
    }
}
...
function getParentCategoryName(parent_id, locale_id, cb){
